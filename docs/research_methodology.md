...
An interview-ready research summary is:

> I reframed an unfinished cube-scanning feature as a controlled perception
> study. I defined hypotheses about calibration, Lab color representation,
> temporal aggregation, and cube-aware assignment constraints. I built a
> grouped data protocol to prevent adjacent-frame leakage, exact-state and
> class-balanced metrics, session-level bootstrap analysis, and an explicit
> failure taxonomy. The important distinction is that a method can have high
> sticker accuracy yet still fail the end-to-end task, so exact 54-sticker
> reconstruction and physical solve success are the primary outcomes.
